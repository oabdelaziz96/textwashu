...
function grader() {
  ...
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var newSheet = ss.getSheetByName("#423");
  var dataArray = newSheet.getRange(2, 4, quantity, 3).getValues();
  ...
  //Prepare for grading
      ...
      for (i=0; i < quantity; i++) {
        ...
        Logger.log("valid =" +curValid);
        
        if (curValid) {  
        curResponse = curResponse.toLowerCase();
        
        Logger.log(curResponse);
        var curAnswerArray = curResponse.match(/[a-z]/g);
        Logger.log(curAnswerArray);
        var curNumAnswers = Math.min(curAnswerArray.length,numQs);
        
        
        Logger.log(curNumAnswers);
        
        grades[i]=0;
        
        for (k=0; k < curNumAnswers; k++) {
          var curCorrect = key.charAt(k) == curAnswerArray[k];
          Logger.log(key.charAt(k));
          Logger.log(curAnswerArray[k]);
          Logger.log(curCorrect);
          if (curCorrect) {
           grades[i]++; 
          }
